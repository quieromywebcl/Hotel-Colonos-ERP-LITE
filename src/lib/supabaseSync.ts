...
export async function fetchAllDataFromSupabase(): Promise<FullAppData> {
  ...
  try {
    ...
    // 3. Fetch Customers / Guests (Try customers table first as required)
    let guests: Guest[] | undefined = undefined;
    try {
      let custRes = await supabase.from('customers').select('*').order('created_at', { ascending: false });
      if (custRes.error || !custRes.data || custRes.data.length === 0) {
        custRes = await supabase.from('guests').select('*').order('created_at', { ascending: false });
      }
      if (!custRes.error && custRes.data) {
        guests = custRes.data.map(mapDbToGuest);
      }
    } catch (e) {
      console.warn('Error al consultar tabla customers/guests:', e);
    }
...
// GUESTS / CUSTOMERS (Save exclusively to public.customers)
