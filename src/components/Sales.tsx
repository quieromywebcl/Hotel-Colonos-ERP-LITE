...
export const Sales: React.FC<SalesProps> = ({
  ...
  const roomPricePerNight = selectedRoomObj ? selectedRoomObj.pricePerNight : 45000;
  ...
  const filteredSales = sales.filter((s) => {
    ...
    return (
      s.saleCode.toLowerCase().includes(query) ||
      s.guestName.toLowerCase().includes(query) ||
      s.guestRut.toLowerCase().includes(query) ||
      s.roomNumber.toLowerCase().includes(query) ||
      (s.vehiclePlate && s.vehiclePlate.toLowerCase().includes(query)) ||
              ...
              {filteredSales.map((sale) => (
                <tr key={sale.id} className="hover:bg-[#3A3A3C]/40 transition">
                  <td className="py-4 px-4 font-mono">
                    <div className="font-extrabold text-[#C29F5C] text-sm">{sale.saleCode}</div>
                  ...
                  <td className="py-4 px-4 font-mono font-extrabold text-[#E5E7EB] text-sm">
                    Hab. {sale.roomNumber} ({sale.roomCategory})
                  </td>

                  <td className="py-4 px-4">
                    <div className="text-neutral-200 font-medium">
                      {formatDateES(sale.checkInDate)} → {formatDateES(sale.checkOutDate)}
                    </div>
                  ...
                  <td className="py-4 px-4">
                    {sale.extraServices && sale.extraServices.length > 0 ? (
                      <div className="space-y-0.5">
                        {sale.extraServices.map((srv, idx) => (
                  ...
                  <td className="py-4 px-4">
                    ...
                    >
                      {sale.paymentMethod} ({sale.paymentStatus === 'paid' ? 'Pagado' : 'Pendiente'})
                    </span>
                  </td>
